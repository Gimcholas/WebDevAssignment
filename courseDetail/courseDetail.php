...
<body>
    ...
    <div class="banner">
        <?php
            $course_section_id = $_GET['course_sectipn_id'];
        ...
        <h1> Announcements </h1>
        <?php
            $sql = "SELECT * FROM announcement WHERE course_section_id = $course_section_id";
        ...
        <div class="eachAnnouncement">
            <img src="
            <?php
                $profile_image_sql = "SELECT profileImagePath FROM user WHERE username=$result['username']";
                $profile_image = mysqli_fetch_assoc(mysqli_query($connect,$profile_image_sql));
                echo $profile_image_path;
            ?>" alt="Author picture">
            <h3>
            <?php
                $author_name_sql = "SELECT * FROM instructor WHERE username=$result['username']";
